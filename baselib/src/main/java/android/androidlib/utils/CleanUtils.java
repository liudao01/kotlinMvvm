...
public final class CleanUtils {
    ...
    public static boolean cleanInternalCache(Context context) {
        return DataCleanManagerUtils.deleteDir(context.getCacheDir());
    }

    public static String getFolderSizeFormat(File file) {
        String formatSize = "";
        try {
            formatSize = DataCleanManagerUtils.getFormatSize(getFolderSize(file));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return formatSize;
        ...
        try {
            ...
            for (int i = 0; i < fileList.length; i++) {
                ...
                if (fileList[i].isDirectory()) {
                    szie = szie + getFolderSize(fileList[i]);
                } else {
                    szie = szie + fileList[i].length();
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
